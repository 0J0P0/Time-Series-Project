...
```{r}
prediction1 = predict(m1_2, n.ahead=12)
pr1 = prediction1$pred
tl1 = pr1 - prediction1$se # límite inferior
tu1 = pr1 + prediction1$se # límite superior
```

```{r}
ts.plot(lnser,tl1,tu1,pr1,lty=c(1,2,2,1),col=c(1,4,4,2),xlim=c(2013,2018),
type="o"); abline(v=2013:2018,lty=3,col=4)
...
```{r}
prediction2 = predict(m2_2, n.ahead=12)
pr2 = prediction2$pred
tl2 = pr2 - prediction2$se # límite inferior
tu2 = pr2 + prediction2$se # límite superior
```

```{r}
ts.plot(lnser,tl2,tu2,pr2,lty=c(1,2,2,1),col=c(1,4,4,2),xlim=c(2013,2018),
type="o"); abline(v=2013:2018,lty=3,col=4)
...
```{r}
inicio=c(2017,12); ultimo=c(2018,12);
obs = window(lnser, start = inicio, end = ultimo)
```
...
pr = prediction$pred
tl = pr - prediction$se # límite inferior
tu = pr + prediction$se # límite superior
