...
plot_ly(x=~time(bcser), y=~resi1,
            type = "scatter", fill="tonexty", mode="lines") %>%
  layout(title='<b> Residuals of model 1 <b>',
         xaxis=list(title='Time'), yaxis=list(title='(Thousand million euros)')) %>%
  add_lines(x=~time(bcser), y=~-3*sd(resi1), fill=F, line=list(color='red', dash='dash')) %>%
  add_lines(x=~time(bcser), y=~3*sd(resi1), fill=F, line=list(color='red', dash='dash'))
