...
```{r}
ts_cor(d1d12bcserEC, lag.max = 72)
```
...
#   xreg = xreg[-1,]
# 
# arima(d1d12bcser,order=c(2,0,0),
...
p2 = ggplotly(ggplot(data=NULL, aes(x=time(d1d12bcser), y=abs(resiEC))) +
  geom_smooth(fill="#2a788e") + 
  geom_point(color="#440154", fill='#414487', shape=21) +
  theme_bw() + 
  xlab("Time") + ylab("abs(residuals 1)") +
...
scatter.smooth(sqrt(abs(resiEC)), lpars = list(col=2))
```
...
```{r}
ggplot(data=NULL, aes(sample=resiEC)) +
  stat_qq_band(color='#1F968BFF', fill='#1F968BFF', alpha=0.3) +
  stat_qq_line(color='#39568CFF') +
  stat_qq_point(color="#440154FF", fill='#3b528b', shape=21) +
  theme_bw() +
...
```{r}
# f = list(f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13,f14,f15,f16,f17,f18,f19,f20,f21,f22,f23,f24)
# n = length(f)
# for (i in 1:n) {
