...
plot_ly() %>%
  ...
  add_lines(x=~time(bcser), y=~-3*sd(resi1), fill=F,
            line=list(color='red',dash='dot'), name='Lower IC limit') %>%
  add_lines(x=~time(bcser), y=~3*sd(resi1), fill=F,
            line=list(color='red', dash='dot'), name="Upper IC limit") %>%
...
ggfig = ggplot(data=NULL, aes(x=time(bcser), y=abs(resi1))) +
  ...
  geom_point(color="darkblue", fill='blue', shape=21) +
  theme_bw() + 
...
plot_ly() %>%
  add_trace(x=~time(bcser), y=~resi2, type = "scatter", fill="tonexty",
        mode="lines",name='Residuals of model 1') %>%
  add_lines(x=~time(bcser), y=~-3*sd(resi2), fill=F,
            line=list(color='red',dash='dot'), name='Lower IC limit') %>%
  add_lines(x=~time(bcser), y=~3*sd(resi2), fill=F,
            line=list(color='red', dash='dot'), name="Upper IC limit") %>%
...
```{r}
# qqPlot(resi1, main="Normal Q-Q Plot", xlab="Theoretical Quantiles", ylab="Sample Quantiles", grid=T)
