...
Aunque la trasnformación logaritmica es de fácil interpretación, comprobamos que no es suficiente, por lo que se ha de proponer un valor de $\lambda$ para la transformación Box-Cox ya mencionada, que tiene la siguiente forma:
...
Tras la transformación, la variancia para los diferentes grupos de observaciones es similar. Además de que la altura de los IQR de los boxplots ya es prácticamente la misma a lo largo del tiempo.

#### Patrón estacional

Para buscar un posible patrón estacional de orden $s=12$, se grafícan las subseries correspondientes a cada año de la serie temporal. Además de dos representaciones adicionales, la primera ... La segunda muestra la distribución de las observaciones por meses.

```{r}
ts_seasonal(bcser, type="all", title=" <b> Seasonality plot for Time Series <b>")
```

Para cada año se observa una clara relación anual, en donde, como se habia comentado anteriormente, en el mes de agosto se ve una clara caida de los valores de las observaciones. Lo que conlleva a tener un IQR por debajo del resto. 

Nos damos cuenta tmabién de que el tercer mes del año tiene muetsras con valores que aumentan y seguidamente caen ligeramente en el abril o viceversa. Aunque estas muestras presentan una distribución con mucha variabilidad dependiendo del año, como se identifica en el tercer gráfico. Posiblemente este fenómeno este relacionado con los efectos de calendario, ya que por esos meses se encuentra la Semana Santa, la cual se distribuye entre ambos meses de manera distinta dependiendo el año. Con lo cual, genera estos aumentos que se detectan en marzo cuando la Semana Santa ocurrio en mayor parte en Abril y por otro lado, los aumentos del mes de abril corresponden a los años para los que la Semana Santa tuvo lugar más en el mes de marzo. Más adelante ase discutirá el tratamiento de dicho efecto.

Se procede a realizar la diferenciación estacional de orden 12. Con lo que el valor del parámetro $D$ toma el valor de la unidad.
...
Tras la transformación se puede afirmar que las subseries presentan una misma media, constante horizontalmente. De la misma forma, ya no es visible una dependencia entre muestras del mismo periodo, con lo que se ha eliminado el patrón estacional.
...
Tras la eliminación del patrón estacional observamos que la serie no presenta signos de una tendencia lineal o algún tipo de tendencia, por lo que podemos considerar ya de entrada la variabilidad como constante. Aunque para comprobar se compararán la variancia de la serie con patrón estacional y su variancia tras una diferenciación regular.
...
Como la variancia de la serie aumenta tras la segunda diferenciación regular, se identifica que se ha realizado una sobrediferenciación, con lo que no es necesaria la aplicación de la segunda trasnformación para asegurar media constante, solo basta con la primera. Obteniendo el parámetro $d=1$.
...
Tras el proceso de transformaciones, únicamente se ha requerido una diferenciación estacional para la serie original. Con lo que la serie estacionaria $W_t$ resulta ser:
