...
```{r}
last=c(2017,12); trainEC=window(serEC, end=last)
```
...
```{r}
f23 = plot_ly(x=~time(serEC), y=~serEC,
        type = "scatter",
        mode = "lines+markers",
        line = list(color="#2a788e"),
        marker = list(color="#2a788e"),
  ...
  add_trace(x = window(time(serEC), start=c(2018,1)),
            y = pr1,
            line = list(color="rgb(68, 1, 84)", dash = "dash"),
            marker = list(color="rgb(68, 1, 84)"),
            name = "Predicted"
            )%>%
  add_ribbons(x = window(time(serEC), start=c(2018,1)),
              ymin=tl1, ymax=tu1,
              line = list(color = 'rgba(34, 168, 132, 0.2)'),
              fillcolor = 'rgba(34, 168, 132, 0.2)',
              name = "95% confidence",
...
```{r}
start=c(2017,12); last=c(2018,12);
obs = window(serEC, start = start, end = last)
```
...
```{r}
cat("AIC Modelo 1: ", AIC(mEC), "\n")
cat("BIC Modelo 1: ", BIC(mEC), "\n")
```
...
```{r}
bcserEC.lin=lineal(bcserEC,mod.atip$atip)

serEC.lin = (lambda*(bcserEC.lin)+1)^(1/lambda)

# plot(serEC.lin, col=2)
# lines(serEC)

f1 = plot_ly(x = time(serEC), y = serEC,
             type = "scatter",
             mode="lines",
             fill="tonexty",
             line = list(color = "#2a788e"),
  ...
  add_trace(x=~time(serEC.lin), y=~serEC.lin,
            type = "scatter",
            name = "serEC.lin",
            mode="lines",
            fill=F,
...
```{r}
f26 = ts_seasonal(bcserEC.lin, type="all", title=" <b> Seasonality plot for Time Series <b>")
f26
```
...
```{r}
f29 = ts_cor(d1d12bcserEC.lin, lag.max = 72)
f29
```

```{r}
arima(d1d12bcserEC.lin,order=c(2,0,0),
           seasonal=list(order=c(0,0,1),period=12))
```


```{r}
# arima(d1d12bcserEC.lin,order=c(1,0,1),
           # seasonal=list(order=c(0,0,1),period=12))
```


```{r}
mOEC = arima(bcserEC.lin,order=c(2,1,0),
           seasonal=list(order=c(0,1,1),period=12))
mOEC
```
...
```{r}
f31 = ggplot(data=NULL, aes(sample=resiEC.lin)) +
  stat_qq_band(color='#1F968BFF', fill='#1F968BFF', alpha=0.3) +
  stat_qq_line(color='#39568CFF') +
  stat_qq_point(color="#440154FF", fill='#3b528b', shape=21) +
  theme_bw() +
...
```{r}
resiEC.lin <- resid(mOEC)
f33 = ts_cor(resiEC.lin, lag.max = 72)
f33
```

```{r}
tsdiag(mOEC,gof.lag=72)
```
...
```{r}
Mod(polyroot(c(1,-mOEC$model$phi)))
```

```{r}
Mod(polyroot(c(1,mOEC$model$theta)))
```

```{r}
f21 = ggplotly(autoplot(mOEC) +
  geom_point(color="#440154", fill='#414487', shape=21, size=1) +
  theme_bw() +
  ggtitle(" <b> Invers roots model <b>") +
  theme(plot.title = element_text(hjust = 0.5)))
...
```{r}
last=c(2017,12); trainEC.lin = window(serEC.lin, end=last)
```

```{r}
bctrainEC.lin = (trainEC.lin^lambda-1)/lambda
mEC_train.lin <- arima(bctrainEC.lin, order=c(2,1,0),
                  seasonal=list(order=c(0,1,1), period=12))
mEC_train.lin
mOEC
```
...
pr1 = (lambda*(preEC.lin$pred)+1)^(1/lambda)
...
```{r}
f23 = plot_ly(x=~time(serEC.lin), y=~serEC.lin,
        type = "scatter",
        mode = "lines+markers",
        line = list(color="#2a788e"),
        marker = list(color="#2a788e"),
  ...
  add_trace(x = window(time(serEC.lin), start=c(2018,1)),
            y = pr1,
            line = list(color="rgb(68, 1, 84)", dash = "dash"),
            marker = list(color="rgb(68, 1, 84)"),
            name = "Predicted"
            )%>%
  add_ribbons(x = window(time(serEC.lin), start=c(2018,1)),
              ymin=tl1, ymax=tu1,
              line = list(color = 'rgba(34, 168, 132, 0.2)'),
              fillcolor = 'rgba(34, 168, 132, 0.2)',
              name = "95% confidence",
...
```{r}
cat("AIC Modelo 1: ", AIC(mOEC), "\n")
cat("BIC Modelo 1: ", BIC(mOEC), "\n")
```
