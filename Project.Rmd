...
La decisión de presentar un ARMA estacional se debe a que las posibildades propuestas contienen un número considerable de parámetros. Tras experimentar con los casos que no superasen el número de parámetros mínimo ya presente (4 del AR(4)), se ha llegado a la conclusión, _analizando la significación de los coeficientes y el AIC de cada opción (estos modelos se han ajustado con las mejores opciones regulares explicadas a continuación)_, de que el ARMA(2,2) y el ARMA(2,1) son los más adecuados junto al AR(4) y el MA(6). Todas las opciones son versátiles ya que pueden explicar retrasos que con opciones más simples no se considerasen y además permiten ser reducidos en caso necesario si algún coeficiente no es significativo.
...
Todos estos modelos tienen un AIC muy similar, por lo que nos fijaremos también en la significación de sus parámetros. 

El primer modelo tiene tres parámetros no significativos: $\Theta_3$, $\Theta_4$ y $\Theta_6$, que coinciden con el previo análisis del ACF, donde se ha visto que el sexto retraso sobresale notablemente pero ni el tercero ni el cuarto ni el quinto lo hacen. 

El segundo modelo propuesto solo tiene un parámetro no significativo $\theta_1$, que tiene un $t$-ratio de aproximadamente $1.64$. A diferencia de los demás, el tercer modelo no presenta ningún parámetro no significsativo, pero cabe mencionar que la significación del parámetro $\Theta_2$ roza el límite con un valor de $2.009$.

El cuarto modelo presenta el parámetro $\theta_1$ no significativo, con un $t$-ratio de aproximadamente $1.58$, y el parámetro $\Phi_1$, que consta con un $t$-ratio cerca de $1.96$ y que podría considerarse como significativo, en funcíon del AIC.
...
```{r}
ts.plot(ser,tl1,tu1,pr1,lty=c(1,2,2,1),col=c(1,4,4,2),xlim=c(2016,2019),
type="o"); abline(v=2016:2019,lty=3,col=4)


myforecast <- forecast(m1_train, h=12)
plot(myforecast, xlim=c(2016,2019))
...
```{r}
ts.plot(ser,tl2,tu2,pr2,lty=c(1,2,2,1),col=c(1,4,4,2),xlim=c(2016,2019),
type="o"); abline(v=2016:2019,lty=3,col=4)
...
MEJORES INTERVALOS EN M2, PERO LA VARIANCIA NO ES CONSTANTE XD
...
```{r}
# f = list(f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13,f14,f15,f16,f17,f18,f19,f20,f21,f22,f23,f24,f25)
# n = length(f)
# for (i in 1:n) {
